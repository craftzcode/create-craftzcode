...
3.  Remove bolierplates, add script for `clean`, running dev with filtering both `docs` and `web`.
    - Remove boilerplates
...
12. Setup AUTH package for Better-Auth.
   ...
   - To handle api requests, you need to set up a route handler on your server there's a many option here [Better-Auth Mound Handler](https://www.better-auth.com/docs/installation#mount-handler) but in this guide we will integrate it using `Next.js` mount handler.
     - First we need to add this `@craftzcode/auth` dependency in `packages/web/package.json`
       ```json
       "@craftzcode/auth": "*"
       ```
     - Second we need to create api route `apps/web/app/api/auth/[...all]/route.ts` then set up a the `better-auth` route handler.
       ```ts
       import { auth } from "@craftzcode/auth"; // path to your auth file
       import { toNextJsHandler } from "better-auth/next-js";
 
       export const { POST, GET } = toNextJsHandler(auth);
       ```
       - GIT COMMIT: `git commit -m "feat(api): integrate better-auth handler"`
     - Setup Protected Procedure Middelware
       - Create a `lib` folder in `packages/api/src` and create a `context.ts` file in `packages/api/src/lib`.
         ```ts
         import { cache } from 'react'
         ...
         ```
         - GIT COMMIT: `git commit -m "feat(api): create tRPC context"`
  ...
  - PULL REQUEST TITLE: `feat(auth, api): integrate better-auth with tRPC`   
